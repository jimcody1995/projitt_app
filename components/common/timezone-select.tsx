  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <span className={cn('truncate')}>{selectedValue || 'Select a timezone'}</span>
          <ButtonArrow />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-(--radix-popper-anchor-width) p-0">
        <Command>
          <CommandInput placeholder="Search timezone..." className="h-[36px]" />
          <CommandList>
              ...
              <CommandGroup>
                {timeZoneList.map(({ value: itemValue, label }) => (
                  ...
                  >
                    ...
                    {value === itemValue && <Check className={cn('size-4 ms-auto')} />}
                  </CommandItem>
                ))}
              </CommandGroup>
            </ScrollArea>
