...
const selectTriggerVariants = cva(
  `
    flex bg-background w-full items-center justify-between outline-none border border-[#bcbcbc] shadow-xs shadow-black/5 transition-shadow 
    text-foreground data-placeholder:text-muted-foreground focus-visible:border-[#0D978B] focus-visible:outline-none focus-visible:ring-[3px] 
    focus-visible:ring-[#0D978B]/30 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1 
  ...
  `,
  {
    variants: {
      size: {
        ...
        md: 'h-8.5 px-3 text-[0.8125rem] leading-(--text-sm--line-height) gap-1 rounded-[10px]',
        lg: 'h-10 px-4 text-sm gap-1.5 rounded-[10px]',
      },
    },
    defaultVariants: {
      size: 'md',
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        'relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 text-sm outline-hidden text-foreground hover:bg-accent focus:bg-[#EBFFF7] data-disabled:pointer-events-none data-disabled:opacity-50',
